function [b,y,w,output] = construct_L1overL2(A,x,lambda)
%written by Min Tao (Nanjing Univeristy)
...
x2 = x/norm(x)^2*norm(x,1);
nx2 = norm(x,2);
for iter = 1:maxIter
    wold = w;
    v = U*(U'*(wold - x2))+x2; % Projection onto rg A'
    error_rangeAt = norm(v-wold);
    w = Ps(v,w0);
